...
def test_percentual_negativo_levanta_erro():
    with pytest.raises(ValueError):
        aplicar_desconto(10000, Desconto("percentual", -20))


def test_percentual_maior_que_100_levanta_erro():
    with pytest.raises(ValueError):
        aplicar_desconto(10000, Desconto("percentual", 150))


def test_valor_negativo_levanta_erro():
    with pytest.raises(ValueError):
        aplicar_desconto(10000, Desconto("valor", -500))


def test_subtotal_zero_nao_levanta_zero_division():
    r = aplicar_desconto(0, Desconto("valor", 500))
    assert r["total"] == 0
    assert r["desconto_pct"] == 0
